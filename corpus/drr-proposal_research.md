...
```mermaid
flowchart LR
    A[Fragmented coordination] --> B[Lower sell-through certainty]
    B --> C[Reduced reinvestment]
    C --> D[Lower capacity and reliability]
    D --> E[Harder buyer adoption]
    E --> A

    C --> F[Weaker handling and cold-chain fit]
    F --> G[More shrink and service risk]
    G --> E

    H[Improved coordination] --> I[Higher certainty]
    I --> J[More reinvestment]
    J --> K[Better capacity and reliability]
    K --> L[Higher repeat utilization]
    L --> I
```
...
```mermaid
flowchart TD
    A[Producer-side test] --> A1[Mean monthly net cash margin >= baseline]
    A --> A2[Volatility metric improves vs baseline]

    B[Buyer-side test] --> B1[Delivered cost per accepted case <= comparator tolerance]
    B --> B2[Service floor met]

    A1 --> C[Feasibility window]
    A2 --> C
    B1 --> C
    B2 --> C

    C --> D[Phase II trigger]
    A1 --> E[Fail if any required condition is not met]
    A2 --> E
    B1 --> E
    B2 --> E
```
