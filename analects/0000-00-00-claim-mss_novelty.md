...
The collapse of ownership-based information structures is not primarily a technical failure; it is a failure of coordination. Information’s natural properties (its infinite recombinability, its resistance to scarcity, and its entropic spread) render ownership models increasingly brittle. What society requires is not greater containment, but better systems of stewardship that align with the laws of information itself. The Mycite Framework proposes such a system.

Mycite is not simply a new software package or protocol. It is a design paradigm for how information can be managed, navigated, and synchronized across diverse domains without requiring central authorities to control its flow. Mycite makes decentralized, contribution-based systems practical, adaptive, and economically viable.

4.1 ULI NOTATION DELIMITERS

Digital computers, in combination with laws of information, provide a stringent guide that one must stay within to create a ULI.

Firstly, consider a sequential  stream of bits. There must be a convention that specifies when a notation ends. Digital computers currently use a convention of “word size”, alternatively is a unary number system that denotes the end of a notation with a zero. However this explodes in the size of space it requires. Despite the problem with its size, this is what is called ‘self-terminating’.

Secondly, consider a sentence’s nature. Part of this nature is that the beginning of a sentence takes on new meaning as the sentence continues. In this way, information is what is called ‘Retrodeterministic’. This means that the meaning of any part of a sentence can’t be understood as intended until the sentence signals that it is finished; either by pause or by contextual conclusion.

Lastly, we consider how alphabetic data schema can allow for polymorphic referencing. This is only seen in a single dimension in an alphabetic language like English. In English we use the 26 letters of the alphabet as elements and indo-european roots as rudimentary components of meaning (in combination with grammar). However if we were to employ an object oriented polymorphic behavior, we could abstract combinations of larger words and root words by referencing a position on a set of all possible combinations. Here we can see how this practice would grow out of control as a result of needing to carry over all previous sets into the next larger set. Similarly this can apply to the letters of the alphabet, in that we have 26 symbols, but if we were to provide a symbol to each of the combinations, we are capable of creating with two of these letters then we would need a number of symbols = 26 + 26^2.
...
By integrating data and metadata into a single, unified system, the framework uses a highly reduced alphabet of elements to embody the components that are arranged to create any instance of information. This is accomplished by using the SSID Layering to create hierarchical abstractions. This approach uses partial-convention, rather than being solely self defining. 

The convention aspects of the MSS are largely given the task of avoiding the redundancies and pitfalls of the ULI delimiters outlined in 4.1. However the other aspects of the structurally semantic ID (SSID) represent an algorithmic way of referencing and recombinning pointers within the same continuous notation. In other words, without the SSID, one would need to describe which particular outcome is desired by singling out the instance that exists amongst all possible arrangements. So a 100 by 100 pixel picture would have a notation size of 10,000, in addition to notating the value of the pictures archetype as 100 by 100 is also a single position on a set of all n by m pictures archetypes, and so on and so forth. This reductive process allows us to follow any given datum’s path of narrative creation down to a set of 10 rudimentary components. 

§ 4.1.2 Mycelium Schema Standardization (MSS):
...
This is also the same reasoning with which the MSS software is made to handle and operate on data with out being explicitly engineered to. This makes the task of software development far faster for any given purpose and context. Rather than having to consider how to visualize data, navigate it, and mediate on it for each case; the Dev process foremost includes considerations for how a user may want to more complexly interface with data, rather than what form it expects information to exist in.  
...
Take agriculture as an example. Consumer interest in local and differentiated produce is real, yet the scalable channels are intermediated and institution-facing, and most food spending goes to post-farm marketing functions rather than to the farm itself. In the 2020 local-food sales picture, institutions and intermediaries accounted for 46 percent of direct local edible food sales, while direct-to-consumer channels accounted for 33 percent. In the Food Dollar framework, farmers received 15.9 cents of each dollar spent on domestically produced food in 2023. Under those conditions, local supply can remain underused not because demand is absent, but because fragmented coordination, logistics, compliance, and administrative overhead make local output harder to procure at scale.
...
This inefficiency extends beyond economics. Centralized control locks innovation behind proprietary walls. A farmer, a mechanic, or a small grocer is excluded from the broader data economy. Not due to lack of value, but because their tools speak an incompatible language.
...
Practically, this allows two regional food cooperatives to share data on forecasts, inventories, or logistics. Even if their systems are entirely distinct. They don’t need to run the same software. They need only share a semantic layer. Governance and privacy are preserved; coordination becomes effortless.
